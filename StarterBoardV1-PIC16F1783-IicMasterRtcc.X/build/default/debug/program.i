...
void programInitialize(void) {
    ...
    i2c_Initialize(32000000, 1200);
}

void programLoop(void) {
    uint8_t bcdSecond = 0;
    uint8_t bcdSecondPrev = 0;
    uint8_t decSecond = 0;
    ...
    while(1) {
        bcdSecond = i2c_MasterByteReadSlave(0x68, 0);

        if(bcdSecond != bcdSecondPrev) {


            bcdMinute = i2c_MasterByteReadSlave(0x68, 1);
            bcdHour = i2c_MasterByteReadSlave(0x68, 2);
            day = i2c_MasterByteReadSlave(0x68, 3);
            bcdDate = i2c_MasterByteReadSlave(0x68, 4);
            bcdMonth = i2c_MasterByteReadSlave(0x68, 5);
            bcdYear = i2c_MasterByteReadSlave(0x68, 6);
            ...
            decMonth = rtcc_ConvertBcdToDec(bcdMonth & 0x1F);
            decYear = rtcc_ConvertBcdToDec(bcdYear);
            decCentury = bcdMonth >> 7;
...
void rtcc_WriteRegister(uint8_t day, uint8_t date,
    ...
    i2c_MasterByteWriteSlave(0x68, 3, day);
    i2c_MasterByteWriteSlave(0x68, 6, rtcc_ConvertDecToBcd(year));
    i2c_MasterByteWriteSlave(0x68, 5, (uint8_t)(rtcc_ConvertDecToBcd(month) | (century << 7)));
    i2c_MasterByteWriteSlave(0x68, 4, rtcc_ConvertDecToBcd(date));
    i2c_MasterByteWriteSlave(0x68, 2, rtcc_ConvertDecToBcd(hour));
    i2c_MasterByteWriteSlave(0x68, 1, rtcc_ConvertDecToBcd(minute));
    i2c_MasterByteWriteSlave(0x68, 0, 0);
}
...
void rtcc_LcdDisplay(uint8_t day, uint8_t date,
    ...
    lcd_Goto(0, 11);
    lcd_PrintDigitInt32(date, 2, 0, 1);
    lcd_PrintChar('/');

    lcd_Goto(0, 14);
    lcd_PrintDigitInt32(month, 2, 0, 1);

    lcd_Goto(1, 12);
    lcd_PrintDigitInt32(century, 2, 0, 1);

    lcd_Goto(1, 14);
    lcd_PrintDigitInt32(year, 2, 0, 1);

    lcd_Goto(1, 0);
    lcd_PrintDigitInt32(hour, 2, 0, 1);
    lcd_PrintChar(':');

    lcd_Goto(1, 3);
    lcd_PrintDigitInt32(minute, 2, 0, 1);
    lcd_PrintChar(':');

    lcd_Goto(1, 6);
    lcd_PrintDigitInt32(second, 2, 0, 1);
